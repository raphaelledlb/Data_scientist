...
puts "Nombre de Handle : #{arr.count}"


puts "Handle le plus court : #{arr.sort_by(&:length)[0]}"


puts "Handle de 5 caractères : #{arr.count {|number| number.length == 6}}"

puts "Handle qui commencent par une majuscule : #{arr.count{|handle| handle[1].match?( /[[:upper:]]/ )}}"


puts "Handle par ordre alphabétique : #{arr.sort}"


puts "Handle triés du plus petit au plus grand : #{arr.sort_by(&:length)}"


puts "Position dans l'array de la personne @epenser : #{arr.index('@epenser') }"
...
def group_size(arr)
	i = 0
	arr.max{|a,b| a.length <=> b.length} #combien de caractère possède le string le plus long (-> 33)
	while (i <= 33) 
		print "nombre de handle avec #{i} caractère :"
		puts arr.count{|a| a.length == i}
		i += 1 
	end
end
